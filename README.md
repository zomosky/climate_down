...
| `build_parser()` | 返回顶层 `argparse.ArgumentParser`，含 `run` / `list-sources` / `list-steps` / `list-variables` / `s2s` 五个 subcommand；想加新子命令在它上面挂即可 |
| `cmd_run(args)` | `run` subcommand 实现，可单独 import 复用（如包成 Airflow operator）|
| `cmd_list_sources(args)` | `list-sources` subcommand 实现 |
| `cmd_list_steps(args)` | `list-steps` subcommand 实现；`args.date=None` 时自动使用 UTC 两天前，`args.cycle` 默认 `12`；`today` / `yesterday` 等符号日期也会在此解析 |
| `cmd_list_variables(args)` | `list-variables` subcommand 实现；`args.date=None` 时自动使用 UTC 两天前，`args.cycle` 默认 `12`，`args.step` 默认 `0`；符号日期同上解析 |
...
`uv sync` 之后 `pyproject.toml` 的 `[project.scripts]` 会注册一个 `climate-download` 命令，覆盖五个 subcommand：
...
| `climate-download list-steps --source <name\|path> [--date YYYYMMDD] [--cycle N] [--json]` | 调 `Source.list_available_steps`，枚举一个 init 在 bucket 里实际发布了哪些 step；`--source` 接受裸名（自动解析 `config/sources/<name>.yaml`）或 YAML 路径；`--date` 和 `--cycle` 均可省略，默认 UTC 两天前 / 12z |
| `climate-download list-variables --source <name\|path> [--date YYYYMMDD] [--cycle N] [--step H] [--json \| --yaml]` | 拉一个 step 的 sidecar，打印所有 `(param, levtype, levelist)` 唯一组合并附 `level_desc`（若源 sidecar 提供，如 wgrib2 idx 的 `"850 mb"` / `"2 m above ground"`）；`--yaml` 直接吐 `variables:` 脚手架，按 levtype 聚合 + 排序 levels + 把 `level_desc` 作为注释列，粘进 job YAML 就能用；三个定位参数均可省略，默认 UTC 两天前 / 12z / step 0 |
...
# 列出 AIFS 发布了哪些 step（默认 UTC 两天前 12z；走 GCS XML LIST）
uv run climate-download list-steps --source aifs
# 0
# 6
# 12
# ...
...
# 快速查看 AIFS 当前默认 init 包含哪些变量（仅需 --source，无需其他参数）
uv run climate-download list-variables --source aifs --json | head -5

# 生成可粘贴到 job YAML 的 variables 脚手架（仅需 --source，无需其他参数）
uv run climate-download list-variables --source aifs --yaml

# 指定具体 init 查询 GFS 变量（JSON 喂下游）
uv run climate-download list-variables --source gfs \
    --date 20260510 --cycle 0 --step 6 --json | jq '.[0:3]'

# 一键生成 GFS 脚手架（指定 init）
uv run climate-download list-variables --source gfs \
